...
export class TransfertService {
  ...
  constructor(private http: HttpClient) {
    this.transfertUrl = 'http://localhost:9191/agent/';
  }
  ...
  public findTransfertByIdAgent(id:any): Observable<Transfert[]> {
    let email = sessionStorage.getItem('email');
    let password = atob(sessionStorage.getItem('password'));
    const headers = new HttpHeaders({
      Authorization: 'Basic ' + btoa(email + ':' + password),
    });
    return this.http.get<Transfert[]>(`${this.transfertUrl}transfert/findAgent/${id}`, {
      headers,
    });
  }
